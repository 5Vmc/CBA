...
public class RenameFile : EditorWindow
{
    ...
    [MenuItem("工具/批量修改文件名字（后缀序号排序）", false, 2)]
    ...
    string[] toolbarValues = { "后缀单排序", "双排序", "替换", "添加前缀" };
    int toolbarIndex = 0;
    private void OnGUI()
    {
        GUILayout.Space(30);
    ...
    void AddPrefix()
    {
        perfixStr = EditorGUILayout.TextField("想添加的前缀", perfixStr);
        if (GUILayout.Button("确认修改", GUILayout.Height(40)))
        {
            if (EditorUtility.DisplayDialog("是否修改", "修改后可通过返回修改返回，但只能返回一次, 是否修改", "确定", "取消"))
            {
        ...
        string GetNewName(string oldName)
        {
            oldName = oldName.Insert(0, perfixStr);
            return oldName;
        }
    }
