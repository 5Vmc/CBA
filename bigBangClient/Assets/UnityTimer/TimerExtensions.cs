...
namespace UnityTimer
{
    public static class TimerExtensions
    {
        ...
        {
            return Timer.Register(null, duration, onComplete, onUpdate, isLooped, useRealTime, behaviour);
        }

        public static T AddTo<T>(this T disposable, GameObject targetObJ) where T : Timer
        {
            if (targetObJ == null)
            {
                return disposable;
            }
            var markTest = targetObJ.GetComponent<DestroyTimerTrigger>();
            if (markTest == null)
            {
                markTest = targetObJ.AddComponent<DestroyTimerTrigger>();
            }
            markTest.AllTimerList.Add(disposable);
            return disposable;
        }
    }
}
