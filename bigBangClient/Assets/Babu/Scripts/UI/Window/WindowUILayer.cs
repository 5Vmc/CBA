...
namespace deVoid.UIFramework
{
    ...
    public class WindowUILayer : AUILayer<IWindowController>
    {
        ...
        [SerializeField] private WindowTopLayer priorityTopLayer = null;
        ...
        public override void Initialize()
        {
            ...
            UIFrame.FitScreen(priorityParaLayer.transform as RectTransform);
            UIFrame.FitScreen(priorityTopLayer.transform as RectTransform);
        }
        ...
        public override void HideScreen(IWindowController screen)
        {
            ...
            else
            {
                ...
                screen.Hide();
                if (!screen.IsPopup && !screen.IsTop)
                {
                    Debug.LogError(
                    string.Format(
        ...
        public override void HideAll(bool shouldAnimateWhenHiding = true)
        {
            ...
            priorityTopLayer.RefreshDarken();
            windowHistory.Clear();
        }
        ...
        public override void ReparentScreen(IUIScreenController controller, Transform screenTransform)
        {
            ...
            else
            {
                if (window.IsTop)
                {
                    priorityTopLayer.AddScreen(screenTransform);
                    return;
                }
        ...
        private void DoShow(WindowHistoryEntry windowEntry, bool showByHistory = false)
        {
            if (CurrentWindow == windowEntry.Screen)
            {
                ...
                CurrentWindow.Hide();
                     ...
                     && !windowEntry.Screen.IsTop
                     )
            {
                CurrentWindow.Hide();
            }

            if (!windowEntry.Screen.IsPopup && !windowEntry.Screen.IsTop)
            {
                windowHistory.Push(windowEntry);
            }
            //AddTransition(windowEntry.Screen);

            if (windowEntry.Screen.IsTop)
            {
                priorityTopLayer.DarkenBG();
            }
            else if (windowEntry.Screen.IsPopup)
            {
